...
app.post("/api/admin/send-guides", asyncRoute(async (req: Request, res: Response) => {
  ...
  const herHTML = fs.readFileSync(path.join(process.cwd(), "server", "templates", "herGuide.html"), "utf8");
  const hisHTML = fs.readFileSync(path.join(process.cwd(), "server", "templates", "hisGuide.html"), "utf8");
